...
class _CardSwiperState<T extends Widget> extends State<CardSwiper>
  ...
  Widget _backItem(BoxConstraints constraints, int index) {
    if (index == widget.numberOfCardsDisplayed) {
      final percentage = (_cardAnimation.total / (widget.threshold * 4)).abs();
      return Positioned(
      ...
      child: AnimatedOpacity(
        ...
        child: Transform.scale(
          ...
          child: ConstrainedBox(
            constraints: constraints,
            child:
                widget.cardBuilder(context, getValidIndexOffset(index)!, 0, 0),
          ),
        ),
      ),
    );
  ...
  int numberOfCardsOnScreen() {
    if (widget.isLoop) {
      return widget.numberOfCardsDisplayed + 1;
    }
